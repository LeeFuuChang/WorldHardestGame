...
class Little_Thing:
    ...
    def Action(self, action):
        if action[0] == 1 and (self.SELF.y - self.Speed) > 0 and self.Can_UP: #UP
            self.SELF.y -= self.Speed
        if action[1] == 1 and (self.SELF.y + self.Speed) < (self.Window.get_height()-self.Height) and self.Can_Down: #Down
            self.SELF.y += self.Speed
        if action[2] == 1 and (self.SELF.x + self.Speed) < (self.Window.get_width()-self.Width) and self.Can_Right: #Right
            self.SELF.x += self.Speed
        if action[3] == 1 and (self.SELF.x - self.Speed) > 0 and self.Can_Left: #Left
            self.SELF.x -= self.Speed
